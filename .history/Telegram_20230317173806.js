...
let resultado = await leerArchivoExcel('./archivo.xlsx');
...
bot.on('callback_query', (query) => {
    ...
    if (data === '1') {
        ...
        bot.sendMessage(chatId, `Seleccionaste productos de calidad 1, hasta luego ${resultado}`);
        console.log({maquina,mes,dia,year})

    }
    if (data === '3') {
        let respuestaString = JSON.stringify(resultado[0])
