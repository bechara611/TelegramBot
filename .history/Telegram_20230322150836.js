...
`);
        ...
        if(data==='3'){
            ...
            maquinas.forEach(async maquinita => {
                ...
                let productos = [];
                let operadores = [];
                respuestaFiltrada = respuestaFiltrada.filter((elemento) => elemento['id_maqempaque2'] == maquinita  && obtenerSemana(new Date(elemento['FECHATEXTO']==numeroSemana)) &&  elemento['calidad'] == 1)
                //    console.log(respuestaFiltrada)
                //    console.log(maquina)
                console.log(numeroSemana)
                console.log(obtenerSemana(new Date(respuestaFiltrada[0]['FECHATEXTO']==numeroSemana)))
                respuestaFiltrada.forEach(async element => {
                    if (!productos.includes(element['NOMBRE_PROD'] + ' ' + element['ESPESOR'] + 'x' + element['ancho'] + 'mm')) {
                        productos.push(element['NOMBRE_PROD'] + ' ' + element['ESPESOR'] + 'x' + element['ancho'] + 'mm')
                    }
