...
bot.on('callback_query', (query) => {
    ...
    if (data === '1') {
        ...
        let e1=0
        respuestaFiltrada=respuestaFiltrada.filter((elemento)=>elemento['id_maqempaque2']==maquina && elemento['DIAMES']==dia  && elemento['MES']==mes && elemento['Año']==year )
        respuestaFiltrada.forEach(element => {
        e1=e1+element['peso_bovina'];
        });
        //let respuestaString = JSON.stringify(resultado)
        bot.sendMessage(chatId, `E1: ${e1} KG`);
    ...
    if (data === '3') {
        let respuestaFiltrada =resultado;
        let e1=0
        respuestaFiltrada=respuestaFiltrada.filter((elemento)=>elemento['base_op']==='223-00133' && elemento['calidad']===1)
        respuestaFiltrada.forEach(element => {
        e1=e1+element['peso_bovina'];
        });
        console.log(e1)
      //  console.log(respuestaFiltrada)
        let respuestaString = JSON.stringify({Hola:'12'})
        bot.sendMessage(chatId, `Seleccionaste desperdicio, hasta luego ${e1} `);
        console.log({maquina,mes,dia,year})
