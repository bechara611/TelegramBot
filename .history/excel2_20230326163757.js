...
async function leerArchivoExcel3(ruta,hoja='') {
    try {
        let vector = []
        // Leer el archivo de Excel como un buffer
        const fileBuffer = await readFile(ruta);
     ...
     //   const hojasDeCalculo = workbook.SheetNames[hoja];
        ...
        // Convertir la hoja de cálculo en formato JSON
        const worksheet = workbook.Sheets[hoja]; // aca puedes poner el nombre de la hoja directamente
        const json = utils.sheet_to_json(worksheet);
...
export { leerArchivoExcel2,leerArchivoExcel3 }
